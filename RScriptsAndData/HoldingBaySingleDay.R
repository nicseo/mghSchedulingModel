...
################ Create smoothed hourly data for one day only

HBSmoothed = function(Vec = HoldingBay[1,2:201] , ResolutionInHours = 1) {
  HourlyOcc = rep(0,24)
  Vec = as.numeric(Vec)
  Slots = ResolutionInHours/(Resolution/60)
  for (i in 1:24){
    HourlyOcc[i] = mean(Vec[Slots*(i-1)+1:i*Slots])
  }
  return(HourlyOcc)
}

Vec = as.numeric(HoldingBay[1,2:201])
mean(Vec[29:32])

test = HBSmoothed()

plot(1:24,HBSmoothed(HoldingBay[1,2:201], 1))
